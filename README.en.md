...
For social science, humanities, and evidence-grounded writing

AI conversation, source management, PDF reading, notes, analysis, reproducible
research workflows, and fully user-defined Agents, in one coherent desktop workspace.
...
Metis is a **local-first** desktop AI research workbench. It is not "a chat window
with a few academic prompts attached." It is a complete research environment that treats
**projects, sources, sessions, artifacts, rules, tool connections, and execution state**
as first-class objects.

Install once, on a single machine, and carry a research task from question formation to
evidence collection, analysis, drafting, revision, and export **without shuttling context
between unrelated tools**. Your research data and personalization configuration stay local
by default, under your full control.

The defining idea of this version is a single sentence:

> Metis does not decide what kind of researcher you are. It gives you a system for defining
> your own research scenarios, Agents, Skills, MCP services, workflows, memory policies,
...
### Zero-preset, fully user-defined

A fresh install ships with five empty definition areas: Scenarios, Agents, Skills, MCP,
and Metis.md rules. Metis does not preconfigure any journal, thesis, funding, monograph,
policy, or presentation workflow. You decide which concepts should exist and how they
should work.

Metis delivers an **editor and a runtime**, not a pile of assumptions about your research.

### Local-first, privacy-respecting

Projects, sessions, notes, artifacts, and personalization definitions are stored locally in
SQLite and on disk. Content leaves your machine only when a feature you actively use
requires it, such as a question you send to your configured model provider.

The API key is handled by the Electron main process and stored with the operating system's
secure storage. It is **never** written into a personalization definition, an exported
bundle, or a renderer-readable settings response.

### Executable scenarios, not a prompt gallery

A Metis scenario is not a prompt. It is a **versioned configuration graph with explicit
dependencies**. A running conversation receives a frozen manifest, so editing a definition
affects a future run, never the task already in flight.

### Research integrity built in, not bypassable

Evidence status, source identity, correction state, provenance, and publishability are
computed by the runtime, not declared by editable scenario or Agent text. These controls
stay automatic, shown as part of the system layer in the Personalization Center, **not
switches a prompt can turn off**.
...
Conversation, library, PDF reading, notes, graph, timeline, comparison view, experiment
view, LaTeX preview, artifact management, and export, all inside the same project context.
...
The Personalization Center starts empty. Every scenario is created or installed by the user,
and every dependency is visible by name. Five user-owned definition areas are available:
Scenarios, Agents, Skills, MCP, and `Metis.md`.
...
PDFs, bibliographic records, collections, tags, reading status, and research artifacts stay
connected to the same project and conversation context.
...
Durable notes keep their title, body, favorites, and project association. The academic design
system ships with matching light and dark themes.
...
- Connect any OpenAI-compatible model provider from the desktop setup screen.
...
- Keep project-level `Metis.md` instructions bound to the project, not in a global prompt
  that silently affects unrelated work.
...
- Read PDFs inside the workbench and move between papers and notes without leaving the app.
...
- Preview LaTeX-oriented output and keep generated artifacts alongside their source
  conversations.
...
Agents are not hard-coded academic personas. You may create a field interviewer, a
...
1. **Write Markdown directly** — edit a `SKILL.md`-style instruction document inside the
   Personalization Center. The fastest path for users who do not want to build a package.
2. **Install a Skill package** — select a ZIP or directory containing instructions, scripts,
   references, schemas, and other assets.
...
field at a time; no hand-written JSON required.
...
a project. Project rules are edited in the Personalization Center and kept with the actual
project workspace.

Legacy `AGENTS.md` content is migrated without discarding your text. Existing project
...
- Archive definitions no longer needed.
...
tool action to request another confirmation. You can instead guide the active task with a
message, or interrupt it at any time.

Autonomy does not mean inventing research state. Evidence status, correction state, source
identity, provenance, and publishability are computed by the runtime, not accepted from
editable scenario text. Those controls stay automatic and are shown in the Personalization
Center as part of the system layer, **not switches a prompt can turn off**.
...
app" warning. Before continuing, verify the asset name and the SHA-256 value shown in the
release notes.
...
secure storage. It is not written into a personalization definition, an exported bundle, or
a renderer-readable settings response.
...
For the Chinese version, see [中文 README](README.md).
