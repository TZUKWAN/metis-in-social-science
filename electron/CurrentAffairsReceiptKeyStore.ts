...
export interface CurrentAffairsReceiptSecretProtector {
...
export interface CurrentAffairsReceiptFsDeps {
  fsyncFd(fd: number): void;
  fsyncDir(dataDir: string): void;
}

const defaultFsDeps: CurrentAffairsReceiptFsDeps = {
...
const KEY_FILE = 'current-affairs-receipt-key.v1.json';
 ...
 */
function decodeKeyFileSafe(filePath: string, protector: CurrentAffairsReceiptSecretProtector): Buffer | null {
...
/**
 * Loads the durable HMAC key for CurrentAffairs receipt signing, protected
 ...
 */
export function loadOrCreateCurrentAffairsReceiptSecret(
  dataDir: string,
  protector: CurrentAffairsReceiptSecretProtector,
  fsDeps: CurrentAffairsReceiptFsDeps = defaultFsDeps,
): Buffer | null {
