...
const api = {
  ...
  providerList: async () => ipcRenderer.invoke('provider:list') as Promise<{ providers: Array<{ id: string; name: string; baseUrl: string; model: string; vision?: boolean }>; activeId: string | null }>,
  providerSave: async (request: { id?: string; name: string; baseUrl: string; apiKey: string; model: string; vision?: boolean }) => ipcRenderer.invoke('provider:save', request) as Promise<{ ok: boolean; id?: string; error?: string }>,
