...
const ZoteroItemSchema = z.strictObject({
  key: z.string(),
  ...
  zoteroUrl: z.string().optional(),
});

const ZoteroItemListResultSchema = z.strictObject({
  query: z.string().optional(),
  total: z.number().int().min(0),
  items: z.array(ZoteroItemSchema),
});

type ZoteroItemListResult = z.infer<typeof ZoteroItemListResultSchema>;

function formatZoteroItemListResult(summary: string, result: ZoteroItemListResult): string {
  const lines = [summary, `Total: ${result.total}`];
  if (result.query) lines.push(`Query: ${result.query}`);
  for (const item of result.items) {
    const authors = item.authors.join(', ') || 'Unknown authors';
    const year = item.year || 'n.d.';
    const venue = item.venue ? ` — ${item.venue}` : '';
    const doi = item.doi ? ` DOI: ${item.doi}` : '';
    const url = item.zoteroUrl ? ` [Zotero: ${item.zoteroUrl}]` : '';
    lines.push(`- "${item.title || 'Untitled'}" by ${authors} (${year})${venue}${doi}${url}`);
  }
  return lines.join('\n');
}

const ZoteroItemDetailSchema = z.strictObject({
  item: ZoteroItemSchema,
  children: z.array(z.unknown()),
});

type ZoteroItemDetail = z.infer<typeof ZoteroItemDetailSchema>;

function formatZoteroItemDetail(result: ZoteroItemDetail): string {
  const item = result.item;
  const lines = [
    `Zotero item: ${item.title || item.key}`,
    `Key: ${item.key}`,
    `Type: ${item.itemType}`,
  ];
  if (item.libraryType) lines.push(`Library: ${item.libraryType} ${item.libraryId ?? ''}`);
  const authors = item.authors.join(', ') || 'Unknown authors';
  lines.push(`Authors: ${authors}`);
  if (item.year) lines.push(`Year: ${item.year}`);
  if (item.venue) lines.push(`Venue: ${item.venue}`);
  if (item.doi) lines.push(`DOI: ${item.doi}`);
  if (item.url) lines.push(`URL: ${item.url}`);
  if (item.abstract) lines.push(`Abstract: ${item.abstract.replace(/\n+/g, ' ').slice(0, 400)}`);
  if (result.children.length > 0) lines.push(`Children: ${result.children.length}`);
  return lines.join('\n');
}

const ZoteroCollectionSchema = z.strictObject({
  key: z.string(),
  version: z.number().int().optional(),
  name: z.string(),
  parentCollectionKey: z.string().optional(),
  itemCount: z.number().int().min(0),
});

const ZoteroCollectionListSchema = z.strictObject({
  total: z.number().int().min(0),
  collections: z.array(ZoteroCollectionSchema),
});

type ZoteroCollectionList = z.infer<typeof ZoteroCollectionListSchema>;

function formatZoteroCollectionList(result: ZoteroCollectionList): string {
  const lines = ['Zotero collections', `Total: ${result.total}`];
  for (const collection of result.collections) {
    const parent = collection.parentCollectionKey ? ` [parent: ${collection.parentCollectionKey}]` : '';
    lines.push(`- "${collection.name}" — key: ${collection.key} — items: ${collection.itemCount}${parent}`);
  }
  return lines.join('\n');
}

const ZoteroDuplicateGroupSchema = z.strictObject({
  type: z.string(),
  key: z.string(),
  items: z.array(ZoteroItemSchema),
});

const ZoteroDuplicateGroupListSchema = z.strictObject({
  totalGroups: z.number().int().min(0),
  groups: z.array(ZoteroDuplicateGroupSchema),
});

type ZoteroDuplicateGroupList = z.infer<typeof ZoteroDuplicateGroupListSchema>;

function formatZoteroDuplicateGroupList(result: ZoteroDuplicateGroupList): string {
  if (result.totalGroups === 0) return 'No duplicate groups found.';
  const lines = [`Zotero duplicate groups: ${result.totalGroups}`];
  for (const group of result.groups) {
    lines.push(`- ${group.type.toUpperCase()}: "${group.key}" — ${group.items.length} items`);
    for (const item of group.items) {
      const authors = item.authors.join(', ') || 'Unknown authors';
      lines.push(`  - ${item.key}: "${item.title || 'Untitled'}" by ${authors} (${item.year || 'n.d.'})`);
    }
  }
  return lines.join('\n');
}

const ZoteroAttachmentSchema = z.strictObject({
  itemKey: z.string(),
...
type ZoteroAttachment = z.infer<typeof ZoteroAttachmentSchema>;

function formatZoteroAttachment(result: ZoteroAttachment): string {
  const lines = [
    `Zotero PDF attachment: ${result.filename || result.attachmentKey}`,
    result.itemTitle ? `Item title: ${result.itemTitle}` : '',
    `Extracted ${result.extractedPages} of ${result.totalPages} pages`,
    '',
    result.text,
  ];
...
// Side-effect-only Zotero tools — fixed safe confirmation, no raw output reflected.
const decodeZoteroImportItem = buildFixedSuccessDecoder('zotero_import_item', 'Zotero import completed');
const decodeZoteroAddTags = buildFixedSuccessDecoder('zotero_add_tags', 'Tags updated');
const decodeZoteroCreateCollection = buildFixedSuccessDecoder('zotero_create_collection', 'Collection created');
const decodeZoteroImportByUrl = buildFixedSuccessDecoder('zotero_import_by_url', 'Zotero import completed');

const decodeZoteroSearch = buildStructuredDecoder(
  'zotero_search',
  'Zotero search results',
  ZoteroItemListResultSchema,
  (r) => formatZoteroItemListResult('Zotero search results', r),
);
const decodeZoteroGetItem = buildStructuredDecoder(
  'zotero_get_item',
  'Zotero item',
  ZoteroItemDetailSchema,
  formatZoteroItemDetail,
);
const decodeZoteroListCollections = buildStructuredDecoder(
  'zotero_list_collections',
  'Zotero collections',
  ZoteroCollectionListSchema,
  formatZoteroCollectionList,
);
const decodeZoteroFindDuplicates = buildStructuredDecoder(
  'zotero_find_duplicates',
  'Zotero duplicates',
  ZoteroDuplicateGroupListSchema,
  formatZoteroDuplicateGroupList,
);
const decodeZoteroReadAttachment = buildStructuredDecoder(
  'zotero_read_attachment',
  'Attachment content',
  ZoteroAttachmentSchema,
  formatZoteroAttachment,
);
...
export function buildBuiltinDecoders(): Map<string, ToolDecoder> {
  ...
  map.set('zotero_import_item', decodeZoteroImportItem);
  map.set('zotero_add_tags', decodeZoteroAddTags);
  map.set('zotero_create_collection', decodeZoteroCreateCollection);
  map.set('zotero_read_attachment', decodeZoteroReadAttachment);
  map.set('zotero_import_by_url', decodeZoteroImportByUrl);
  map.set('zotero_search', decodeZoteroSearch);
  map.set('zotero_get_item', decodeZoteroGetItem);
  map.set('zotero_list_collections', decodeZoteroListCollections);
  map.set('zotero_find_duplicates', decodeZoteroFindDuplicates);
