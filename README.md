...
- [界面一览](#-界面一览)
- [核心功能详解](#-核心功能详解)
  - [① 对话式 AI 协作——多模型、可引导（科研项目与选题 Chatbot 内置）](#-对话式-ai-协作多模型可引导科研项目与选题-chatbot-内置)
...
### 🧪 选题工作台——从研究兴趣到开题，内含协同 Chatbot

独立的「协同对话」一级工作区已并入选题：在选题工作区一键打开 **Chatbot 协作视图**，
与豆包 / Kimi / GLM / ChatGPT / Claude / DeepSeek 分屏协作——左边交流思路，
右边让 Metis 干活。会话按项目分组；流式输出带推理过程折叠；斜杠命令、
学习技能、一键命名对话；对话可直接引导正在执行的场景。
科研项目的「聊天」页签同样是完整的对话式 AI 工作区。
...
### ① 对话式 AI 协作——多模型、可引导（科研项目与选题 Chatbot 内置）

对话式 AI 不再是独立的一级工作区：它内置于**科研项目的「聊天」页签**与
**选题工作区的 Chatbot 协作视图**（2026-09-05 产品收敛）。
...
- **PPT 构建引擎（Gorden PPT Skill，2026-09-11 集成）**：做 PPT 的默认挂载技能——在对话里直接说"做一个开题答辩 PPT"，自动挂载并用 21 套中文模板生成真实 .pptx（只换文字不破坏排版，产出文件可一键导入成果工作台）；技能资产按需自举获取并支持增量更新，模板为第三方非商业授权（个人学习研究用途）；也可在能力库中检视源仓库（GordenSun/GordenPPTSkill）；
...
│  ├── 成果工作台 (Word/PPT/表格/PDF) ├── 选题工作台 (含 Chatbot 协作)   │
...
4. **开始研究**：在科研项目「聊天」或选题 Chatbot 里发起任务；论文写完去「投稿」让参谋陪你选刊。
