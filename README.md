...
把 AI 对话、文献管理、PDF 阅读、笔记、成果编辑、可复现的研究工作流，
...
### AI 场景编译器：对话式构建，能力自动获取

在场景配置助手里用自然语言描述需求，AI 编译器会**分五个阶段逐步构建**场景：基本信息 →
交付物结构 → 连续工作流 → 场景规则 → 输出计划。每个阶段先联网检索资料、再逐条写入，
右侧编辑器实时逐块成型；构建完成自动保存，离开未保存会弹窗提醒。

工作流需要而本地尚未安装的技能或 MCP 服务，编译器会**自动搜索技能/MCP 市场、核验并
安装**，再绑定到对应步骤——全过程可见、可审计、可卸载。构建结束还有一道自检审计：
空缺扫描、schema 严格校验、质量缺口回喂修复。
...
对话式分阶段构建场景（先检索后写入、逐块上屏、自动保存），缺失技能/MCP 自动搜索市场并安装绑定，构建后自检审计。
...
成果页按项目管理 Word/PPT 文档、版本与媒体。Word/PPT 使用统一的成果模型编辑，支持排版、
版本保存、AI 协作、DOCX/PPTX 导入导出；科研项目页的成果区只提供列表、状态和版本历史查看。
...
- **成果文档模型与自研 codec**：在成果工作台内编辑 Word/PPT，并进行 DOCX/PPTX 导入导出
- **pdfjs-dist**：PDF 渲染与文本提取
- **Zod**：全链路运行时契约校验
- **Vitest**：2800+ 单元/集成/E2E 测试
...
# 生产构建（typecheck + 主进程编译 + 渲染端打包 + 原生模块重编译）
npm run build
...
> 说明：`better-sqlite3` 为原生模块，开发（Node）与打包（Electron）所需的 ABI 不同，
> 分别使用 `npm run rebuild:node` 与 `npm run rebuild:electron` 切换。
...
- **成果管理**：成果页 → 选择项目与成果 → Word/PPT 编辑、版本与导出
- **项目成果查看**：科研项目 → 研究成果 → 查看成果状态与版本历史（只读）
