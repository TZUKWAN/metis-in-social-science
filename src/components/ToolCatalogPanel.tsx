...
function permissionLevel(tool: ToolSpec): 'read' | 'write' | 'execute' | 'network' {
  ...
  if (tool.name.includes('search') || tool.name.includes('lookup') || tool.name.includes('import') || tool.name.includes('zotero') || tool.name.includes('arxiv') || tool.name.includes('crossref') || tool.name.includes('openalex')) return 'network';
