...
export default function ProviderProfilesSection() {
  ...
  const save = useCallback(async () => {
    ...
    if (!form.name.trim() || !form.baseUrl.trim() || !form.model.trim() || (form.keyMode === 'replace' && !form.apiKey.trim())) {
    ...
    const request: ProviderProfileSaveRequest = {
      ...
      ...(form.keyMode === 'replace' ? { newApiKey: form.apiKey.trim() } : {}),
    };
    try {
  ...
  const testConnection = useCallback(async () => {
    ...
    if (form.keyMode === 'saved' && (!selectedProfile || !selectedProfile.isActive)) {
      setNotice({ kind: 'info', message: zh ? '请先切换到此连接，或重新输入 API 密钥后再测试。' : 'Switch to this connection first, or enter a replacement API key to test it.' });
      return;
    }
    if (form.keyMode === 'replace' && !form.apiKey.trim()) {
  ...
  return (
    <section className="settings-group provider-profiles" aria-labelledby="provider-profiles-title">
      <div className="settings-section-heading">
        ...
        <button type="button" className="btn-sm btn-secondary" onClick={startNew} disabled={isBusy} data-testid="provider-profile-new">
      ...
      <div className="provider-profiles__layout">
        ...
        <div className="provider-profiles__editor">
          <label className="settings-label">
            ...
            <input className="settings-input" value={form.name} onChange={(event) => patchForm({ name: event.target.value })} disabled={isBusy} autoComplete="off" />
          </label>
          <label className="settings-label">
            {zh ? 'API 地址' : 'API base URL'}
            <input className="settings-input" value={form.baseUrl} onChange={(event) => patchForm({ baseUrl: event.target.value })} disabled={isBusy} autoComplete="off" spellCheck={false} />
          </label>
          <label className="settings-label">
            {zh ? '模型名称' : 'Model'}
            <input className="settings-input" value={form.model} onChange={(event) => patchForm({ model: event.target.value })} disabled={isBusy} autoComplete="off" spellCheck={false} />
          </label>
          ...
          <label className="settings-label">
            ...
            {form.keyMode === 'saved' ? (
              <div className="settings-key-row">
                <span className="settings-key-mask" aria-label={zh ? '已保存的 API 密钥' : 'Saved API key'}>••••••••</span>
                <button type="button" className="btn-sm btn-secondary" onClick={() => patchForm({ keyMode: 'replace', apiKey: '' })} disabled={isBusy}>{zh ? '更换密钥' : 'Replace key'}</button>
              </div>
            ) : (
              <input type="password" className="settings-input" value={form.apiKey} onChange={(event) => patchForm({ apiKey: event.target.value })} disabled={isBusy} autoComplete="off" />
            )}
          </label>
      ...
      {notice && <div className={`provider-profiles__notice provider-profiles__notice--${notice.kind}`} role={notice.kind === 'error' ? 'alert' : 'status'} aria-live="polite">{notice.message}</div>}
