...
import {
  ...
  isAgentRunStatus,
  isLikelyGoalFeedback,
  now,
  partsFromExecutionEnvelopes,
  reduceExecutionEnvelope,
  toGoalCardData,
  toRightPanelTaskStatus,
  validArtifactTypes,
  AGENT_EXECUTION_TURN_BUFFER_LIMIT,
  SCENARIO_CATALOG_MAX_ATTEMPTS,
  SCENARIO_CATALOG_RETRY_DELAY_MS,
  type ArtifactItem,
  type ArtifactItemType,
  type ChatMessage,
} from './chat/chatPageModel';
// ChatMessage 等类型由 chatPageModel 持有（2026-09-15 拆分）；既有
// `import type { ChatMessage } from '../../pages/ChatPage'` 的调用方经此处
// 再导出保持路径不变。
export type { ChatMessage } from './chat/chatPageModel';
import type { GoalCardData } from '../components/GoalCardInline';
import { isInternalExecutionCopy } from '../presentation/executionCopy';
import type { AgentActivityStatus } from '../components/AgentActivityTimeline';
import {
...
export default function ChatPage({ renderLayout, uiMode, intentRevision = 0, previewMode = 'inline' }: ChatPageProps) {
  ...
  const rightPanelArtifacts = aggregateRightPanelArtifacts(projectOutcomes, artifacts);
