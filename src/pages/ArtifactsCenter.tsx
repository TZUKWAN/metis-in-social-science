 ...
 * 类别（论文/源代码/元数据/参考文献），右键可标记（草稿/待审核/
 * 部分完成/最终版/已过期）。标记通过 researchReview 持久化到产物
 * 的 reviewStatus，并展示每个产物的版本历史。
 */
...
type MarkStatus = 'draft' | 'pending' | 'partial' | 'verified' | 'stale';

const MARK_ORDER: Array<{ status: MarkStatus; labelKey: string }> = [
  { status: 'draft', labelKey: 'artifactsCenter.markDraft' },
  { status: 'pending', labelKey: 'artifactsCenter.markPending' },
  { status: 'partial', labelKey: 'artifactsCenter.markPartial' },
  { status: 'verified', labelKey: 'artifactsCenter.markVerified' },
  { status: 'stale', labelKey: 'artifactsCenter.markStale' },
];
...
export default function ArtifactsCenter({ projectId }: ArtifactsCenterProps) {
  ...
  const [menu, setMenu] = useState<{ x: number; y: number; row: ArtifactRow } | null>(null);
  ...
  const mark = async (row: ArtifactRow, status: MarkStatus) => {
    if (!projectId || markBusy) return;
    setMarkBusy(true);
    setNotice(null);
    try {
      const result = await window.metis?.researchReview?.({
        operation: 'review',
        reviewKind: 'artifact',
        projectId,
        entityId: row.artifact.id,
        expectedVersion: row.artifact.version,
        toStatus: status,
        reason: t('artifactsCenter.marked', { mark: t(MARK_ORDER.find((m) => m.status === status)?.labelKey ?? 'artifactsCenter.markDraft') }),
      });
      if (result?.success) {
        setNotice(t('artifactsCenter.marked', { mark: t(MARK_ORDER.find((m) => m.status === status)?.labelKey ?? 'artifactsCenter.markDraft') }));
        await load();
      } else {
        // 最终版（verified）受真实性层约束：缺少可核验交付配置时后端拒绝。
        const code = result && 'code' in result && typeof (result as { code?: unknown }).code === 'string'
          ? (result as { code: string }).code
          : 'unknown';
        setNotice(code === 'rejected'
          ? t('artifactsCenter.markRejected')
          : code === 'conflict'
            ? t('artifactsCenter.markConflict')
            : t('artifactsCenter.markFailed'));
      }
    } catch {
      setNotice(t('artifactsCenter.markFailed'));
    } finally {
      setMarkBusy(false);
      setMenu(null);
    }
  ...
  return (
    <div className="artifacts-center" data-testid="artifacts-center">
      <header className="artifacts-center__header">
        ...
        <div className="artifacts-center__filters" role="group" aria-label={t('artifactsCenter.title')}>
          ...
          <button
            ...
            data-testid="artifacts-submissions-open"
          >
            {t('submissions.entryButton')}
          </button>
        </div>
      </header>

      {notice && <div className="artifacts-center__notice" role="status">{notice}</div>}
      ...
      {menu && (
        <div
          ref={menuRef}
          className="artifacts-center__menu"
          role="menu"
          aria-label={t('artifactsCenter.mark')}
          style={{ left: menu.x, top: menu.y }}
          data-testid="artifacts-center-menu"
        >
          <div className="artifacts-center__menu-title">{t('artifactsCenter.mark')}</div>
          {MARK_ORDER.map((entry) => (
            <button
              key={entry.status}
              type="button"
              role="menuitem"
              className={`artifacts-center__menu-item ${menu.row.artifact.reviewStatus === entry.status ? 'active' : ''}`}
              disabled={markBusy}
              onClick={() => void mark(menu.row, entry.status)}
            >
              {t(entry.labelKey)}
            </button>
          ))}
      ...
      {submissionsOpen && (
        <SubmissionsPanel projectId={projectId} onClose={() => setSubmissionsOpen(false)} />
      )}
    </div>
  );
}
