...
function StepCard({
  ...
  const { zh, draft, definitions, mutateDraft, toggleStepResource, acquire, addStep, removeStep } = props;
  ...
  return (
    <li className="scenario-focus-step" style={{ '--scenario-step-depth': depth } as React.CSSProperties} data-testid="sw-workflow-step">
      <article
        ...
        onDrop={(event) => { event.preventDefault(); onDrop(step.id); }}
      >
        <header className="scenario-focus-step__head">
          <span className="scenario-focus-step__drag" aria-label={zh ? '拖动排序' : 'Drag to reorder'}><GripVertical size={15} /></span>
          ...
          />
          <button type="button" className="btn-secondary btn-sm" onClick={() => addStep(step.id)} data-testid={`sw-add-substep-${step.id}`}>
            <Plus size={13} /> {zh ? '子步骤' : 'Sub-step'}
          </button>
          <button type="button" className="scenario-focus-step__remove" aria-label={zh ? '删除步骤' : 'Remove step'} onClick={() => removeStep(step.id)}>
            <Trash2 size={14} />
          </button>
        </header>
        <label>
          <span>{zh ? '专属 Prompt' : 'Dedicated prompt'}</span>
          <textarea rows={4} value={step.prompt ?? ''} onChange={(event) => update({ prompt: event.target.value })} data-testid="sw-step-prompt" />
        </label>
        <label>
        ...
        <section className="scenario-focus-step__resources" aria-label={zh ? '步骤资源' : 'Step resources'}>
          <div>
            ...
            <span className="scenario-focus-step__resource-actions">
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('skill', step.id, 'package')} data-testid={`sw-step-import-package-${step.id}`}>{zh ? '本地导入' : 'Import local'}</button>
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('skill', step.id, 'url')} data-testid={`sw-step-url-skill-${step.id}`}>URL</button>
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('skill', step.id, 'search')} data-testid={`sw-step-search-skill-${step.id}`}>{zh ? '在线搜索' : 'Search online'}</button>
            </span>
            <div className="scenario-focus-step__resource-list">
              {skills.map((skill) => <label key={skill.id}><input type="checkbox" checked={step.skillIds.includes(skill.id)} onChange={() => toggleStepResource(step.id, 'skill', skill.id)} />{skill.name}</label>)}
          ...
          <div>
            <strong>MCP</strong>
            <span className="scenario-focus-step__resource-actions">
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('mcp', step.id, 'package')} data-testid={`sw-step-import-mcp-${step.id}`}>{zh ? '本地导入' : 'Import local'}</button>
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('mcp', step.id, 'url')} data-testid={`sw-step-url-mcp-${step.id}`}>URL</button>
              <button type="button" className="btn-secondary btn-sm" onClick={() => acquire('mcp', step.id, 'search')} data-testid={`sw-step-search-mcp-${step.id}`}>{zh ? '在线搜索' : 'Search online'}</button>
            </span>
            <div className="scenario-focus-step__resource-list">
              {mcps.map((mcp) => <label key={mcp.id}><input type="checkbox" checked={step.mcpIds.includes(mcp.id)} onChange={() => toggleStepResource(step.id, 'mcp', mcp.id)} />{mcp.name}</label>)}
...
export default function ScenarioFocusedEditor(props: Props) {
  const { zh, draft, mutateDraft, ensureDeliverable, addStep, reorderSteps } = props;
    ...
    <div className="scenario-focus" data-testid="sw-focused-editor">
      <section className="scenario-focus__section" data-testid="sw-page-basics">
        ...
        <div className="scenario-focus__grid scenario-focus__grid--basic">
          <label><span>{zh ? '场景名称' : 'Scenario name'}</span><input value={draft.name} onChange={(event) => mutateDraft((scenario) => { scenario.name = event.target.value; })} data-testid="sw-config-name" /></label>
          <label><span>{zh ? '触发短语（每行一条）' : 'Trigger phrases (one per line)'}</span><textarea rows={3} value={draft.triggerPhrases.join('\n')} onChange={(event) => mutateDraft((scenario) => { scenario.triggerPhrases = lineValues(event.target.value); })} /></label>
        </div>
      </section>
      ...
      <section className="scenario-focus__section" data-testid="sw-page-structure">
        ...
        <div className="scenario-focus__grid scenario-focus__grid--four">
          <label><span>{zh ? '总篇幅' : 'Total length'}</span><input value={draft.deliverable?.globalLength ?? ''} placeholder={zh ? '例如 12000 字' : 'e.g. 12,000 words'} onChange={(event) => mutateDraft((scenario) => { ensureDeliverable(scenario); scenario.deliverable!.globalLength = event.target.value || undefined; })} data-testid="sw-config-length" /></label>
          <label><span>{zh ? '一级章节数量' : 'Top-level chapters'}</span><input type="number" min={0} max={24} value={chapterCount} onChange={(event) => updateChapterCount(event.target.value)} data-testid="sw-chapter-count" /></label>
          <label><span>{zh ? '二级章节（每章最少）' : 'Second-level minimum'}</span><input type="number" min={0} max={24} value={secondary.min} onChange={(event) => updateSecondaryRange('min', event.target.value)} data-testid="sw-secondary-min" /></label>
          <label><span>{zh ? '二级章节（每章最多）' : 'Second-level maximum'}</span><input type="number" min={0} max={24} value={secondary.max} onChange={(event) => updateSecondaryRange('max', event.target.value)} data-testid="sw-secondary-max" /></label>
          <label><span>{zh ? '语言' : 'Language'}</span><select value={draft.deliverable?.language ?? 'zh'} onChange={(event) => mutateDraft((scenario) => { ensureDeliverable(scenario); scenario.deliverable!.language = event.target.value === 'en' ? 'en' : 'zh'; })}><option value="zh">{zh ? '中文' : 'Chinese'}</option><option value="en">{zh ? '英文' : 'English'}</option></select></label>
        </div>
        <div className="scenario-focus__outline" aria-label={zh ? '当前结构预览' : 'Current structure preview'}>
          ...
          {chapters.map((chapter, index) => <div key={chapter.id} className="scenario-focus__chapter"><label><span>{zh ? `第 ${index + 1} 章` : `Chapter ${index + 1}`}</span><input value={chapter.title} onChange={(event) => updateChapter(chapter.id, { title: event.target.value })} /></label><label><span>{zh ? '二级章节数' : 'Sections'}</span><input type="number" min={0} max={24} value={(chapter.children ?? []).length} onChange={(event) => updateChildCount(chapter, event.target.value)} /></label><ol>{(chapter.children ?? []).map((child) => <li key={child.id}><input value={child.title} onChange={(event) => updateChapter(chapter.id, { children: (chapter.children ?? []).map((item) => item.id === child.id ? { ...item, title: event.target.value } : item) })} /></li>)}</ol></div>)}
        </div>
      </section>
      ...
      <section className="scenario-focus__section scenario-focus__section--workflow" data-testid="sw-page-capability">
        ...
        <label className="scenario-focus__workflow-prompt"><span>{zh ? '工作流总 Prompt' : 'Workflow prompt'}</span><textarea rows={5} value={draft.workflowPrompt ?? ''} onChange={(event) => mutateDraft((scenario) => { scenario.workflowPrompt = event.target.value; })} placeholder={zh ? '例如：前一步成果必须成为后一步的输入；只有达到完成标准才能推进；最终形成全部交付物。' : 'For example: carry forward prior results, require completion before progression, and finish all deliverables.'} data-testid="sw-workflow-prompt" /></label>
        <div className="scenario-focus__workflow-actions"><button type="button" className="btn-secondary btn-sm" onClick={() => addStep()} data-testid="sw-workflow-add"><Plus size={13} />{zh ? '新增步骤' : 'Add step'}</button><small>{zh ? '拖动步骤或子步骤调整执行顺序。' : 'Drag steps or sub-steps to reorder execution.'}</small></div>
        <ol className="scenario-focus__steps" data-testid="sw-workflow-list">{rootSteps.map((step) => <StepCard key={step.id} step={step} depth={0} props={props} onDragStart={setDraggedStepId} onDrop={(targetId) => { if (draggedStepId && draggedStepId !== targetId) reorderSteps(draggedStepId, targetId); setDraggedStepId(null); }} />)}{rootSteps.length === 0 && <li className="scenario-focus__empty">{zh ? '尚未配置工作流。可以新增步骤，或在左侧对话中描述期望流程。' : 'No workflow yet. Add a step or describe the desired flow in the conversation on the left.'}</li>}</ol>
      </section>
