...
describe('AgentLoop ToolPresentation attack matrix', () => {
  ...
  it('does not leak raw errors on HITL denial', async () => {
    ...
    expect(providerContent).not.toContain('/tmp/x.txt');
    const toolMsg = result.messages.find((m) => m.role === 'tool');
    expect(toolMsg).toBeDefined();
    expect(toolMsg!.content).toBe('Tool execution failed');
    expect(toolMsg!.name).toBe('write_file');
  });
...
function makeZoteroItem(text: string): Record<string, unknown> {
  return {
    key: 'zotero-item-1',
    version: 1,
    libraryType: 'user',
    ...
    venue: 'Journal of Examples',
    tags: ['example'],
    zoteroUrl: 'https://www.zotero.org/groups/123/items/ABC',
  };
}

function makeZoteroSearchResult(text: string): string {
  return JSON.stringify({ query: 'test query', total: 1, items: [makeZoteroItem(text)] });
}

function makeZoteroItemDetail(text: string): string {
  return JSON.stringify({ item: makeZoteroItem(text), children: [] });
}

function makeZoteroCollectionList(text: string): string {
  return JSON.stringify({
    total: 1,
    collections: [
      {
...
function makeZoteroDuplicateGroupList(text: string): string {
  return JSON.stringify({
    totalGroups: 1,
    groups: [
      {
        type: 'title',
        key: text,
        items: [makeZoteroItem(text)],
      },
    ],
  });
}

function makeZoteroAttachment(text: string): string {
  return JSON.stringify({
    itemKey: 'zotero-item-1',
...
function makeStructuredInput(toolName: string, text: string): string {
  switch (toolName) {
    ...
    case 'zotero_search':
      return makeZoteroSearchResult(text);
    case 'zotero_get_item':
      return makeZoteroItemDetail(text);
    case 'zotero_list_collections':
      return makeZoteroCollectionList(text);
    case 'zotero_find_duplicates':
      return makeZoteroDuplicateGroupList(text);
    case 'zotero_read_attachment':
      return makeZoteroAttachment(text);
...
describe('AgentLoop builtin presenter matrix (102 cases)', () => {
  ...
  it('has 51 built-in decoders registered', () => {
    expect(builtinDecoders.size).toBe(51);
  });
...
describe('REVIEW-450 P0-2: Structured DTO hostile content blocking', () => {
  ...
  it('RED: blocks local path in zotero item url field', () => {
    const hostile = JSON.stringify({
      total: 1, items: [{
        key: 'k1', itemType: 'journalArticle', title: 'Paper', authors: ['A'],
        url: '/home/alice/secret.pdf', version: 1,
      }],
    });
    const result = probeDecoder('zotero_search', hostile);
    expect(result.content).not.toContain('/home/alice');
  });
  ...
  it('RED: blocks path in zotero_read_attachment text field', () => {
    const hostile = JSON.stringify({
      itemKey: 'k1', attachmentKey: 'a1', filename: 'doc.pdf', itemTitle: 'Paper',
      totalPages: 10, extractedPages: 10, text: 'Secret at C:\\Users\\Bob\\private.txt',
    });
    const result = probeDecoder('zotero_read_attachment', hostile);
    expect(result.content).not.toContain('C:\\Users');
  });
