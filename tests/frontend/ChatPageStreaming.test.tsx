...
function mockMetis(overrides: Record<string, unknown> = {}) {
  const handlers: { stream?: (data: { sessionId?: string; content: string; reasoning?: string; isFinished: boolean }) => void } = {};
  const metis = {
    ...
    agentChat: vi.fn(),
    agentControl: vi.fn(),
    onChatStreamChunk: vi.fn((cb: (data: { sessionId?: string; content: string; reasoning?: string; isFinished: boolean }) => void) => {
      handlers.stream = cb;
      return () => {};
    }),
    ...overrides,
